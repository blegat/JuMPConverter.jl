...
@constraint(model, KKT1, 0 <= sum{h in 1..H}(E[s,w,h]*xx[w,h]) + mu[s] - R[s,w] + 2./beta[s] * (y[s,w]-y[s,0])
                                                    complements y[s,w] >= 0)
@constraint(model, KKT2, 0 <= mu[s]  complements y[s,0] >= 0)
@constraint(model, coupl, eta[w] + sum{s in 1..S}(y[s,w]) >= S*alpha[w])
@objective(model, Max, sum{s in 1..S, w in 1..W}(rho[s]*(R[s,w]-C[s,w])*y[s,w])
