...
function build_model()
    ...
    @constraint(model, E7, - 2*X7 + SQR(X7) >= -1)
    @constraint(model, E8, 2*X7 + SQR(X7) >= -1)
    @constraint(model, E9, SQR(X3) - 2*X3*X7 + SQR(X7) >= 0)
    @constraint(model, E10, SQR(X3) + 2*X3*X7 + SQR(X7) >= 0)
