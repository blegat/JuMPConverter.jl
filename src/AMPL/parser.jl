...
"""
    DatSchema(param_ndims::OrderedDict{String,Int})
    DatSchema(model::JuMPConverter.Model)

Minimal info needed to parse an AMPL `.dat` file: a map from parameter
name to number of indexing dimensions. Generated by the `.mod` parser
and embedded in the generated `.jl` file so that loading a `.dat` at
